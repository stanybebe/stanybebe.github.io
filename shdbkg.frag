...
vec2 hash( vec2 x )
{
    ...
    x = x*k + k.yx;
    return -1.0 + 5.0*sin( (u_time*.4)*2.0 * k*fract( x.x*x.y*(x.x+x.y)) );
}
...
void main()
{
    ...
    vec2 r = vec2(gl_FragCoord.xy+ 0.5*u_resolution.xy );
	 float n= noise(.5*uv);
    float n2= noise(.5*uv)*cos(n*u_time*.003)*2.;
    ...
    float mody = mod(width-width2+n2,fract(floor(r.y+u_time*.04)*3.));
